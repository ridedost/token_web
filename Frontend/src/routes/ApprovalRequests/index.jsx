...
const ApprovalRequest = () => {
  ...
  const fetchAllSendRequests = async (currentPage) => {
    ...
    try {
      if (role === '1') {
        const response = await allSendRequestForAdmin(currentPage, token);
        if (response.status === 200) {
          console.warn(response);
          setAllSendRequests(response.data?.itemsToSend);
        ...
        const response = await allSendRequestForVendor(currentPage, token);
        if (response.status === 200) {
          console.warn(response);
          setAllSendRequests(response.data?.data);
          setTotalPages(response.data.totalPages);
          setRoutes(true);
        } else {
          setAllSendRequests([]);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      if (role === '1') {
        ...
        if (response.status === 200) {
          dispatch(setFetching(false));
          toast.success('Approve Request send !');
      ...
      dispatch(setFetching(false));
      toast.error('Approve Request Not Send !');
    }
  ...
  const handleForward = async (id) => {
    ...
    try {
      ...
      dispatch(setFetching(false));
      toast.error('Request Not Forward !');
    }
    // Handle the approval logic here
  };
  ...
  const handleReturn = async (id) => {
    ...
    try {
      ...
      dispatch(setFetching(false));
      toast.error('Request Not Return!');
    }
    // Handle the approval logic here
  };
  ...
  return (
    <div className="table-subContainer">
            ...
            <table className="tables">
              ...
              <tbody className="table-body">
                {allSendRequests?.length > 0 ? (
                  allSendRequests?.map((user, index) => (
      ...
      {showReject ? (
        <RejectModal setShowReject={setShowReject} rejectId={rejectId} />
      ) : (
        ''
      )}
      {showEditVendor ? (
