...
          return res.status(400).json({ error: "Coupon is not valid. Please enter a valid coupon code." });
        }
        console.log(data[0]._id.toString(),couponValid[0].userID)
       if(data[0]._id.toString()!==couponValid[0].userID){
        return res.status(400).json({ error: "Coupon is not valid. Please enter a valid coupon code." });
       }
...
const allsendor = allReq.filter((e, i) => {
        return (
              (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId &&e.sendor.status=="requested"&&  e.superAdmin.status=="pending"&&e.receiver.status=="pending")||
              (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId &&e.sendor.status=="requested"&& e.superAdmin.status=="forwarded"&& e.receiver.status=="pending")||
              (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId &&e.sendor.status=="requested"&& e.superAdmin.status=="requestedback"&& e.receiver.status=="accepted")||
              (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId &&e.sendor.status=="pending"&& e.superAdmin.status=="returning"&& e.receiver.status=="accepted")||

              (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId === e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId &&e.sendor.status=="requested"&&  e.superAdmin.status=="pending"&&e.receiver.status=="pending")||
              (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId === e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId &&e.sendor.status=="pending"&&  e.superAdmin.status=="accepted"&&e.receiver.status=="pending")||

               (e.sendor.vendorId===_id&&e.superAdmin.adminId === e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="requested"&&  e.superAdmin.status=="pending"&&e.receiver.status=="pending")||
               (e.sendor.vendorId===_id&&e.superAdmin.adminId === e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="pending"&&  e.superAdmin.status=="pending"&&e.receiver.status=="accepted")
        );
      });
...
      return (
  
          (e.superAdmin.status == "returning"&& e.sendor.status !== "rejected") ||
        
        e.superAdmin.status == "requestedback" ||
       (e.superAdmin.status == "forwarded"&&   e.receiver.status !== "rejected")||
     
       (e.superAdmin.adminId === e.sendor.vendorId &&
          e.superAdmin.status === "pending" && e.sendor.status === "requested" && e.receiver.status!="rejected")||
          (e.superAdmin.adminId === e.sendor.vendorId &&
...
    if(data.sendor.vendorId!==data.receiver.vendorId && data.superAdmin.adminId!=data.receiver.vendorId&&data.superAdmin.adminId!=data.sendor.vendorId&&data.sendor.status=="pending" &&   data.superAdmin.status=="returning" &&   data.receiver.status=="accepted" ){
      data.sendor.status = "rejected";
      data.sendor.reject_region = req.body.reject_region;
      data.superAdmin.reject_region = req.body.reject_region;
      data.receiver.reject_region = req.body.reject_region;
...
    if(data.sendor.vendorId!==data.receiver.vendorId && data.superAdmin.adminId===data.receiver.vendorId&&data.superAdmin.adminId!=data.sendor.vendorId&&data.sendor.status=="requested" &&   data.superAdmin.status=="pending" &&   data.receiver.status=="pending" ){
      console.log("bjfbjhbhjbhfbjhfsjhjhfbhjf")
      data.superAdmin.status = "rejected";
      data.sendor.reject_region = req.body.reject_region;
      data.superAdmin.reject_region = req.body.reject_region;
      data.receiver.reject_region = req.body.reject_region;
...
      return res.status(200).json({ message: "Successfully rejected" });
    }
    if(data.sendor.vendorId!==data.receiver.vendorId && data.superAdmin.adminId!==data.receiver.vendorId&&data.superAdmin.adminId===data.sendor.vendorId&&data.sendor.status=="pending" &&   data.superAdmin.status=="pending" &&   data.receiver.status=="accepted" ){
      data.sendor.status = "rejected";
      data.sendor.reject_region = req.body.reject_region;
      data.superAdmin.reject_region = req.body.reject_region;
      data.receiver.reject_region = req.body.reject_region;
...
   const rejectedsendorlist = await VendorSettlement.find({
  "sendor.vendorId": _id,
...
  const all = rejectedsendorlist.filter((e, i) => {
        return (
               (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.receiver.status=="rejected")||
               (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="rejected")||
               (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId === e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.superAdmin.status=="rejected")||
               (e.sendor.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId === e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="rejected")||
               (e.sendor.vendorId===_id&&e.superAdmin.adminId === e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="rejected")||
               (e.sendor.vendorId===_id&&e.superAdmin.adminId === e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.receiver.status=="rejected")
              
  ...
  const rejectedreceiverlist=await VendorSettlement.find ({
      "receiver.vendorId":_id,

    
  })
console.log("listlist",rejectedreceiverlist)

    const allReq = rejectedreceiverlist.filter((e, i) => {
          return (
                 (e.receiver.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.receiver.status=="rejected")||
                 (e.receiver.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="rejected")||

                 (e.receiver.vendorId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId === e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.superAdmin.status=="rejected")||
    ...
    const rejectedadminlist=await VendorSettlement.find ({
        "superAdmin.adminId":_id,
      
      
    })
    const allReqs = rejectedadminlist.filter((e, i) => {
            return (
                   (e.superAdmin.adminId===_id &&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.receiver.status=="rejected")||
                   (e.superAdmin.adminId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="rejected")
        
        //          ||  (e.superAdmin.adminId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId === e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.superAdmin.status=="rejected")||
        //            (e.superAdmin.adminId===_id&&e.superAdmin.adminId !== e.sendor.vendorId &&e.superAdmin.adminId === e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="rejected")||
        //            (e.superAdmin.adminId===_id&&e.superAdmin.adminId === e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.sendor.status=="rejected")||
        //            (e.superAdmin.adminId===_id&&e.superAdmin.adminId === e.sendor.vendorId &&e.superAdmin.adminId !== e.receiver.vendorId&&e.sendor.vendorId!==e.receiver.vendorId && e.receiver.status=="rejected")
                  
...
const rejected=[...all,...allReq,...allReqs]
console.log(allReq)
