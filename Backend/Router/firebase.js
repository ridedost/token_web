...
firebaseAdmin.initializeApp({
      ...
      function sendNotification(type, user, title, msg) {
        ...
        const formattedTime = new Date(Date.now()).toLocaleTimeString([], { hour: 'numeric', minute: 'numeric' });
        return new Promise((resolve, reject) => {
          var notifications = firebaseAdmin.database().ref("notifications/" + user);
          notifications.push({title: title,msg: msg,time: formattedTime,type: type})
            .then(() => {
              resolve(true);
            })
            .catch((error) => {
